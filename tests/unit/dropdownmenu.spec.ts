...
describe('DropDownMenu.vue', () => {
  ...
  it('It has constructed the hidden dropdown `vdda-container` placeholder', () => {
    expect(wrapper.contains('.vdda-container')).toBe(true)
    expect(wrapper.contains('.vdda-dropdown-list')).toBe(true)
  })

  it('button click (with simple context) should show 3 dropdown items', () => {
    divParent.click();
    expect(wrapper.findAll('.vdda-dropdown-item').length).toBe(3)
  })
  ...
  it('whilst the dropdown is visible lets add another item and make sure it is there', () => {
    ...
    expect(wrapper.findAll('.vdda-dropdown-item').length).toBe(4)
  })

  it('button click should collapse the dropdown', () => {
    divParent.click();
    expect(wrapper.findAll('.vdda-dropdown-item').length).toBe(0)
  })

  it('dropdown item click should collapse the dropdown', () => {
    divParent.click();  // to open the dropdown (to populate it)
    expect(wrapper.findAll('.vdda-dropdown-item').length).toBe(4)
    expect(wrapper.find('.vdda-dropdown-item')).toBeDefined();
    
    let xx = wrapper.find('.vdda-dropdown-item');
    console.log(xx.html());
    ...
    //   expect(wrapper.findAll('.vdda-dropdown-item').length).toBe(0)  
    // }, 500);
    //console.log(wrapper.find('.vdda-dropdown-item'))
    
    //wrapper.find('.vdda-dropdown-item').trigger('click');
    //expect(wrapper.findAll('.vdda-dropdown-item').length).toBe(0)
